...
class ShipProxy(protocol.Protocol):
    ...
    def sendCryptoPacket(self, data):
        if self.c4crypto is not None and self.peer is not None:
            if logPackets:
                with open('packets/%i.constucted.%s.bin' % (self.packetCount, self.transport.getPeer().host),
                          'wb') as f:
                    f.write(data)
            if verbose:
    ...
    def dataReceived(self, data):
        if verbose: print(
        "[ShipProxy] [%i] Received data from %s!" % (self.packetCount, self.transport.getPeer().host,))
        ...
        while len(self.readBuffer) >= 8:
            ...
            if verbose: print("[ShipProxy] [%i] Received packet with size %i, id %x:%x" % (
            self.packetCount, packetSize, packetType[0], packetType[1]))
            ...
            if logPackets:
                ...
                if self.myUsername is not None:
                    ...
                    self.myUsername, self.connTimestamp, self.packetCount, packetType[0], packetType[1],
                    ...
                    self.orphans.append(
                        {'data': pData, 'count': self.packetCount, 'type': packetType[0], "sub": packetType[1]})

            try:
                packet = packets.packetList[packetType](self, packet)
            except KeyError:
                if verbose: print("[ShipProxy] No packet function for id %x:%x, using default functionality..." % (
                packetType[0], packetType[1]))
            ...
            if logPackets:
                if self.myUsername is not None and len(self.orphans) > 0:
                    ...
                    while len(self.orphans) > 0:
                        ...
                        path = 'packets/%s/%s/%i.%x-%x.%s.bin' % (
                        self.myUsername, self.connTimestamp, oPacket['count'], oPacket['type'], oPacket['sub'],
                        self.transport.getPeer().host)
                        try:
                            os.makedirs(os.path.dirname(path))
                        except exceptions.OSError:
...
def main():
    ...
    if myIp == "0.0.0.0":
        ...
        print(
        "Please edit cfg/pso2proxy.config.json and change myIpAddr to your IP public IP address (Not LAN address if you're on a LAN!) ")
