    ...
    if user.playerId in data.clients.connectedClients:
        ...
        if 'globalChat' not in clientPrefs:
            ...
            if clientPrefs['globalChat'] == True:
                user.sendCryptoPacket(packetFactory.ChatPacket(user.playerId,
                                                               "[Proxy] {red}Global chat is enabled, use |g <Message> to chat and |goff to disable it.").build())
            else:
                user.sendCryptoPacket(packetFactory.ChatPacket(user.playerId,
                                                               "[Proxy] {red}Global chat is disabled, use |gon to enable it and use |g <Message> to chat.").build())
...
def chat(context, params):
    ...
    if not data.clients.connectedClients[context.playerId].getPrefs()['globalChat']:
        context.sendCryptoPacket(packetFactory.ChatPacket(context.playerId,
