...
def create_preferences(client):
    if client.playerId in data.clients.connectedClients:
        user_prefs = data.clients.connectedClients[client.playerId].get_prefrences()
...
def get_chat_packet(context, packet):
    if context.playerId in data.clients.connectedClients:
        user_prefs = data.clients.connectedClients[context.playerId].get_prefrences()
    ...
    return packetFactory.ChatPacket(player_id, "%s *" % translator.translate(message, "en"), channel_id)