...
class OpCommand(Command):
    def call_from_client(self, client):
        if len(self.args.split(" ")) < 2:
            client.send_crypto_packet(
                packetFactory.SystemMessagePacket("[Proxy] {red}Not enough arguments. Usage: %sgop <segaid>",
                                                  0x3).build())
            return
    ...
    def call_from_console(self):
        if len(self.args.split(" ")) < 2:
            return "[ShipProxy] Not enough arguments. Usage: %sgop <segaid>"
        player = self.args.split(" ")[1]
        if not config.is_admin(player):
            current_admins = config.globalConfig.get_key('admins')
            current_admins.append(player)
...
class DeopCommand(Command):
    def call_from_client(self, client):
        if len(self.args.split(" ")) < 2:
            client.send_crypto_packet(
                packetFactory.SystemMessagePacket("[Proxy] {red}Not enough arguments. Usage: %sgdeop <segaid>",
                                                  0x3).build())
            return
        player = self.args.split(" ")[1]
    ...
    def call_from_console(self):
        if len(self.args.split(" ")) < 2:
            return "[ShipProxy] Not enough arguments. Usage: %sgdeop <segaid>"
        player = self.args.split(" ")[1]
        if not config.is_admin(player):
            current_admins = config.globalConfig.get_key('admins')
            current_admins.remove(player)
...
class Ban(Command):
    def call_from_client(self, client):
        ...
        if len(args) < 3:
            client.send_crypto_packet(packetFactory.SystemMessagePacket(
                "[Command] {red}Invalid usage! Proper usage, %sgban <segaid/pid> <value>", 0x3).build())
            return
        if args[1] == "segaid":
    ...
    def call_from_console(self):
        args = self.args.split(' ')
        if len(args) < 3:
            return "[Command] Invalid usage! Proper usage, %sgban <segaid/pid> <value>"
        if args[1] == "segaid":
            if config.is_segaid_banned(args[2]):
            ...
            return "[Command] Invalid usage! Proper usage, >>> %sg<segaid/pid> <value>"
...
class Unban(Command):
    def call_from_client(self, client):
        ...
        if len(args) < 3:
            client.send_crypto_packet(packetFactory.SystemMessagePacket(
                "[Command] {red}Invalid usage! Proper usage, %sgunban <segaid/pid> <value>", 0x3).build())
            return
        if args[1] == "segaid":
    ...
    def call_from_console(self):
        args = self.args.split(' ')
        if len(args) < 3:
            return "[Command] Invalid usage! Proper usage, %sgunban <segaid/pid> <value>"
        if args[1] == "segaid":
            if not config.is_segaid_banned(args[2]):
...
class Kick(Command):
    ...
    def call_from_console(self):
        args = self.args.split(' ')
        if len(args) < 2:
            return "[Command] Invalid usage! Proper usage: %sgkick <playerId>"
        if int(args[1]) in data.clients.connectedClients:
            if data.clients.connectedClients[int(args[1])].get_handle() is not None:
                data.clients.connectedClients[int(args[1])].get_handle().send_crypto_packet(
...
class GlobalMessage(Command):
    def call_from_client(self, client):
        ...
        if len(self.args.split(' ', 1)) < 2:
            client.send_crypto_packet(packetFactory.SystemMessagePacket("[ShipProxy] {red}Incorrect usage. Usage: %sgglobalmsg  <Message>", 0x3).build())
            return
    ...
    def call_from_console(self):
        message = None
        if len(self.args.split(' ', 1)) < 2:
            return "[ShipProxy] Incorrect usage. Usage: %sg[message_type] <Message>"
        try:
...
class Profiler(Command):
    def call_from_console(self):
        ...
        if profile is None:
            ...
            profile.enable()
            SMPacket = packetFactory.SystemMessagePacket("[Proxy NOTICE] Profiling mode has now been enabled. This may cause lag until it has been disabled.", 0x0).build()
            for client in data.clients.connectedClients.values():
                if client.get_handle() is not None:
                    client.get_handle().send_crypto_packet(SMPacket)
            ...
            out.close()
            profile = None
            SMPacket = packetFactory.SystemMessagePacket("[Proxy NOTICE] Profiling mode has now been disabled. Any lag caused by this should subside.", 0x0).build()
            for client in data.clients.connectedClients.values():
                if client.get_handle() is not None:
                    client.get_handle().send_crypto_packet(SMPacket)
